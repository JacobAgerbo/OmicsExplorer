...
WorkDir=`pwd -P`
SnakeMakeFile=`echo ${WorkDir}/03_metagenomics.smk`
ConfigFile=`echo ${WorkDir}/03_config.yaml`
...
no_Assemblies=`ls ${WorkDir}/03_Assembly/03_Assembly/{sample}/contigs.fasta | wc -l`

if [ $no_RAW -ne $no_Assemblies ]
	then
    echo "OmicsExplorer could not find same number of"
		echo "samples between raw data repo and single assemblies"
		exit 0
fi
